...
void main() {
	...
	for (unsigned int i = 0; i < 4; i++)
		if ((Events[i] = CreateEventA(NULL, NULL, NULL, NULL)) == 0) {
			cout << "Не удалось создать событие \"" << i << "\"\n";
			ExitProcess(1);
		}
	if ((EndEvent = CreateEventA(NULL, TRUE, NULL, NULL)) == 0) {
	...
	for (unsigned int i = 0; i < n; i++)
		if ((Threads[i] = CreateThread(NULL, NULL, Employee, NULL, NULL, NULL)) == 0) {
